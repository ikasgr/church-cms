...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-600 mb-4">
        ...
        <span class="text-gray-800">Edit Berita</span>
    </div>
    <h2 class="text-2xl font-bold text-gray-800">
        <i class="fas fa-edit text-blue-600 mr-2"></i>Edit Berita: <?= $news['title'] ?>
    </h2>
</div>

<div class="bg-white rounded-lg shadow-md">
    <form action="<?= base_url('admin/berita/edit/' . $news['id']) ?>" method="POST" enctype="multipart/form-data" class="p-6">
        <?= csrf_field() ?>
...
<?= old('content', $news['content']) ?></textarea>
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="meta_keywords" class="block text-sm font-medium text-gray-700 mb-2">Keywords</label>
                        ...
                        <div>
                            <label for="meta_description" class="block text-sm font-medium text-gray-700 mb-2">Description</label>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="category" name="category" required
                        ...
                        <option value="renungan" <?= old('category', $news['category']) == 'renungan' ? 'selected' : '' ?>>Renungan</option>
                        ...
                        <label for="featured_image" class="cursor-pointer">
                            <div id="imagePreview" class="mb-3">
                                <?php if (!empty($news['featured_image'])): ?>
                                    <img src="<?= base_url('uploads/news/' . $news['featured_image']) ?>" alt="Featured Image" class="max-w-full h-auto rounded-lg shadow-md inline-block">
                                <?php else: ?>
                                    <i class="fas fa-image text-4xl text-gray-400"></i>
                                <?php endif; ?>
                            </div>
                            <p class="text-sm text-gray-600">Klik untuk upload mengganti gambar</p>
                ...
                <div class="border-t pt-6 space-y-3">
                    <h4 class="text-sm font-semibold text-gray-700">Status Publikasi</h4>
                    <label class="flex items-center cursor-pointer">
                        <input type="checkbox" name="is_published" value="1"
                               class="form-checkbox text-blue-600 rounded"
                               <?= old('is_published', $news['is_published']) ? 'checked' : '' ?>>
                        <span class="ml-2 text-sm text-gray-700">
                            <i class="fas fa-eye text-green-600"></i> Publikasikan
                        </span>
                    </label>
                    <div class="bg-gray-50 rounded-lg p-3 text-xs text-gray-600">
                        <p><strong>Status saat ini:</strong> <?= $news['is_published'] ? 'Dipublikasikan' : 'Draft' ?></p>
                        <?php if (!empty($news['published_at'])): ?>
                            <p><strong>Dipublikasikan:</strong> <?= date('d M Y H:i', strtotime($news['published_at'])) ?></p>
                        <?php endif; ?>
                        <p><strong>Terakhir diperbarui:</strong> <?= date('d M Y H:i', strtotime($news['updated_at'])) ?></p>
                    </div>
                </div>
            </div>
        </div>
