        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select id="role" name="role" required
                    ...
                    <option value="seller" <?= old('role', $user['role']) == 'seller' ? 'selected' : '' ?>>Seller - Pengelola UMKM</option>
                </select>
            ...
            <div id="seller-section" class="space-y-4" style="display: none;">
                <div class="border-t pt-6">
                    <h4 class="text-md font-semibold text-gray-800 mb-4 flex items-center gap-2">
                        ...
                        Hubungkan Pelapak UMKM
                    </h4>
                    <p class="text-sm text-gray-600 mb-4">Pilih pelapak UMKM yang akan terhubung dengan akun seller ini.</p>
                </div>
                ...
                <div>
                    ...
                    <select id="seller_id" name="seller_id"
                        ...
                        <?php foreach ($sellers as $seller): ?>
                            <option value="<?= $seller['id'] ?>" <?= old('seller_id', $currentSeller['id'] ?? '') == $seller['id'] ? 'selected' : '' ?>>
                                <?= esc($seller['business_name']) ?> (<?= esc($seller['owner_name']) ?>)
                                <?php if ($seller['status']): ?> - Status: <?= esc(ucfirst($seller['status'])) ?><?php endif; ?>
                                <?php if (!empty($seller['user_id']) && (int) $seller['user_id'] !== (int) ($currentSeller['id'] ?? 0)): ?>
                                    - Terhubung ke User #<?= $seller['user_id'] ?>
                                <?php endif; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
